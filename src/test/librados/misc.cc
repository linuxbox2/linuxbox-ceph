...
TEST_F(LibRadosMiscPP, BigAttrPP) {
  ...
  for (int i=0; i<1000; i++) {
    bl.clear();
    got.clear();
    bl.append(ceph::buffer::raw::create(MIN(g_conf->osd_max_attr_size, 1024)));
    char n[10];
...
TEST_F(LibRadosMiscPP, CopyPP) {
  ...
  // do a big object
  bl.append(ceph::buffer::raw::create(g_conf->osd_copyfrom_max_chunk * 3));
  bl.zero();
  bl.append("tail");
  blc = bl;
  xc = x;
