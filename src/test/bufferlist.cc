...
TEST(Buffer, constructors) {
  ...
  if (ceph_buffer_track)
    EXPECT_EQ(0, ceph::buffer::get_total_alloc());
  {
    bufferptr ptr(ceph::buffer::create(len));
    EXPECT_EQ(len, ptr.length());
    if (ceph_buffer_track)
      EXPECT_EQ(len, (unsigned)ceph::buffer::get_total_alloc());
  }
  //
  // buffer::claim_char
  //
    ...
    EXPECT_EQ(0, ceph::buffer::get_total_alloc());
  {
    ...
    if (ceph_buffer_track)
      EXPECT_EQ(zc_len, (unsigned)ceph::buffer::get_total_alloc());
    ::close(fd);
    ::unlink("testfile");
  }
#endif
...
TEST(BufferList, crc32c_append_perf) {
  ...
  int base_cached = ceph::buffer::get_cached_crc();
  int base_cached_adjusted = ceph::buffer::get_cached_crc_adjusted();
  ...
  assert(ceph::buffer::get_cached_crc() == 0 + base_cached);
  {
    utime_t start = ceph_clock_now(NULL);
    uint32_t r = bla.crc32c(0);
    utime_t end = ceph_clock_now(NULL);
  ...
  assert(ceph::buffer::get_cached_crc() == 1 + base_cached);
  ...
  assert(ceph::buffer::get_cached_crc() == 1 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 1 + base_cached_adjusted);
  {
    utime_t start = ceph_clock_now(NULL);
    uint32_t r = bla.crc32c(5);
    utime_t end = ceph_clock_now(NULL);
  ...
  assert(ceph::buffer::get_cached_crc() == 1 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 2 + base_cached_adjusted);
  ...
  assert(ceph::buffer::get_cached_crc() == 2 + base_cached);
  ...
  assert(ceph::buffer::get_cached_crc() == 3 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 3 + base_cached_adjusted);
  bufferlist ac;
  ac.push_back(a);
  ac.push_back(c);
  {
  ...
  assert(ceph::buffer::get_cached_crc() == 4 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 3 + base_cached_adjusted);
  ...
  assert(ceph::buffer::get_cached_crc() == 5 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 4 + base_cached_adjusted);
  {
    utime_t start = ceph_clock_now(NULL);
  ...
  assert(ceph::buffer::get_cached_crc() == 5 + base_cached);
  assert(ceph::buffer::get_cached_crc_adjusted() == 6 + base_cached_adjusted);

  cout << "crc cache hits (same start) = " << ceph::buffer::get_cached_crc() << std::endl;
  cout << "crc cache hits (adjusted) = " << ceph::buffer::get_cached_crc_adjusted() << std::endl;
}
