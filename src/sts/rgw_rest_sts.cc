...
sts_err::sts_err(struct req_state *s) : rgw_err(s), type(Unknown)
{
  request_id.generate_random();
}


void sts_err::dump() const
{
  char encoded_uuid[38];
  ...
  s.formatter->open_object_section_in_ns("ErrorResponse",
			"http://s3.amazonaws.com/doc/2011-06-15/");
    s.formatter->open_object_section("Error");
      s.formatter->dump_string("Type", error_types[type]);
      if (!s3_code.empty())
	s.formatter->dump_string("Code", s3_code);
      if (!message.empty())
	s.formatter->dump_string("Message", message);
    s.formatter->close_section();
    s.formatter->dump_string("RequestId", encoded_uuid);
  s.formatter->close_section();
}
...
int RGWHandler_REST_STS::init(RGWRados *store, struct req_state *s,
